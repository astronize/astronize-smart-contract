...
contract AdminKAP20Router is Authorization, IAdminKAP20Router {
  ...
  modifier onlyAllowedAddress() {
    require(_allowedAddr.contains(msg.sender), "AdminKAP20Router: Restricted only allowed address");
    _;
  }

  modifier onlyCommittee() {
    require(committee == msg.sender, "AdminKAP20Router: Restricted only committee");
    _;
  }
  ...
  // onlySuperAdminOrAdmin removed
  function addAddress(address _addr) external override {
  ...
  function revokeAddress(address _addr) external override {
  ...
  // onlySuperAdmin removed
  function withdrawKUB(address _to, uint256 _amount) external {
  ...
  function internalTransferKKUB(
    ...
    require(KYC.kycsLevel(_from) >= bitkubNextLevel && KYC.kycsLevel(_to) >= bitkubNextLevel, "Only internal purpose");
