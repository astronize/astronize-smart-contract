...
contract TSXFANMEET is KAP721 {
    ...
    function mint(address _to, uint256 _tokenId) external onlySuperAdminOrOwner whenNotPaused {
        _mint(_to, _tokenId);
    }

    function burn(uint256 _tokenId) external onlySuperAdminOrOwner whenNotPaused {
        _burn(_tokenId);
    }
    ...
    function mintWithMetadata(
        ...
        uint256 _tokenId
    ) external onlySuperAdminOrOwner whenNotPaused {
    ...
    function mintWithMetadataBatch(
        ...
        require(_tokenURIs.length == _tokenIds.length, "BitkubKAP721: need all input in same length");
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _mintWithMetadata(_to, _tokenURIs[i], _tokenIds[i]);
        }

        emit MintWithMetadataBatch(_to, _tokenURIs, _tokenIds);
    }
